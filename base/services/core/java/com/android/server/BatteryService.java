...
//#ifdef VENDOR_EDIT
//licx@OnLineRD.framework.for_breath_light, add
...
public final class BatteryService extends SystemService {
    ...
    abstract class SettingsObserver extends ContentObserver {
        private String mTarget;
        public SettingsObserver(Handler handler, String target) {
            super(handler);
            mTarget = target;
            ContentResolver resolver = mContext.getContentResolver();
            resolver.registerContentObserver(Settings.System.getUriFor(mTarget), false, this);
            update();
        }

        @Override
        public void onChange(boolean selfChange) {
            if (DEBUG_LED) Slog.d(TAG_LED, mTarget + " has been changed, update to " + selfChange);
            update();
        }

        public void update() {
            ContentResolver resolver = mContext.getContentResolver();
            boolean on = Settings.System.getInt(resolver, mTarget, 0) != 0;
            if (DEBUG_LED) Slog.d(TAG_LED, "Now update " + mTarget + ", value is " + on);
            onUpdate(on);
        }

        abstract void onUpdate(boolean on);
    }
    //#endif /* VENDOR_EDIT */
    ...
    private final class Led {
        ...
        //#ifdef VENDOR_EDIT
        //Jevons@OnLineRD.framework.for_breath_light, 2014-02-19
        private final int mBatteryChargingARGB = 0xFF00005A;
        ...
        private SettingsObserver[] mSettingsObservers = new SettingsObserver[2];
        ...
        private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (action.equals(Intent.ACTION_SCREEN_ON)) {
                    mScreenOn = true;
                    mLightHandler.removeMessages(MSG_UPDATE_LIGHT);
                    updateLightsLocked();
                } else if (action.equals(Intent.ACTION_SCREEN_OFF)) {
                    mScreenOn = false;
                    mLightHandler.sendEmptyMessageDelayed(MSG_UPDATE_LIGHT, DELAY_UPDATE_LIGHT);
                }
            }
        };
        //#endif /* VENDOR_EDIT */
        ...
        public Led(Context context, LightsManager lights) {
            ...
            //#ifdef VENDOR_EDIT
            //licx@OnLineRD.framework.for_breath_light, add
            mNotificationLightStateDetector = lights.getLight(LightsManager.LIGHT_ID_NOTIFICATIONS);
            //new LightsService.LightStateDetector(lights, LightsService.LIGHT_ID_NOTIFICATIONS);
        ...
        public void updateLightsLocked() {
            //#ifdef VENDOR_EDIT
            //licx@OnLineRD.framework.for_breath_light, add
            if (DEBUG_LED) Slog.d(TAG_LED, "ScreenOn = " + mScreenOn + ";mBatteryLowHint = " + mBatteryLowHint + ";mChargingHint = " + mChargingHint);
            if (mScreenOn) {
                mBatteryLight.turnOff();
                return;
            }
            if (mLight.getLightState(LightsManager.LIGHT_ID_NOTIFICATIONS))
                return;
            //#endif /* VENDOR_EDIT */
            final int level = mBatteryProps.batteryLevel;
            final int status = mBatteryProps.batteryStatus;
            if (DEBUG_LED) Slog.d(TAG_LED, "level = " + level + ";status = " + status);
            //#ifndef VENDOR_EDIT
            //licx@OnLineRD.framework.for_breath_light, modify
            /*
            ...
            if (status == BatteryManager.BATTERY_STATUS_CHARGING) {
                if (mChargingHint) {
                    ...
                    //mBatteryLight.setColor(mBatteryLowARGB);
                    //#else
                    mBatteryLight.setFlashing(mBatteryChargingARGB, LightsManager.LIGHT_FLASH_TIMED,
                            mBatteryLedOn, mBatteryLedOff);
                    //#endif /*VENDOR_EDIT*/
                } else {
                    // No lights if disable charging hint
                    mBatteryLight.turnOff();
                }
                ...
                if (mBatteryLowHint && level < mLowBatteryWarningLevel) {
                    // Flash red when battery is low and not charging
                    mBatteryLight.setFlashing(mBatteryLowARGB, LightsManager.LIGHT_FLASH_TIMED,
                            mBatteryLedOn, mBatteryLedOff);
                } else {
                    // No lights if not charging and not low
                    mBatteryLight.turnOff();
