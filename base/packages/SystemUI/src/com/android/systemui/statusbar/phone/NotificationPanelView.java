...
public class NotificationPanelView extends PanelView implements
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        resetDownStates(event);
		Log.v("luozexian","onTouchEvent down1111");
        if ((!mIsExpanding || mHintAnimationRunning)
                && !mQsExpanded
                && mStatusBar.getBarState() != StatusBarState.SHADE) {
            Log.v("luozexian","onTouchEvent down2222");
            mAfforanceHelper.onTouchEvent(event);
        }
        if (mOnlyAffordanceInThisMotion) {
			Log.v("luozexian","onTouchEvent down3333");
            return true;
        }
        if (event.getActionMasked() == MotionEvent.ACTION_DOWN && getExpandedFraction() == 1f
                && mStatusBar.getBarState() != StatusBarState.KEYGUARD && !mQsExpanded
                && mQsExpansionEnabled) {
            Log.v("luozexian","onTouchEvent down4444");
        ...
        if (mExpandedHeight != 0) {
			Log.v("luozexian","onTouchEvent down5555");
            handleQsDown(event);
        }
        if (!mTwoFingerQsExpand && mQsTracking) {
			Log.v("luozexian","onTouchEvent down6666");
            onQsTouch(event);
