...
public class FilterEntryDo extends VzwTlv {
    ...
    public FilterEntryDo(AidRangeDo aid_range, AidMaskDo aid_mask,
            RoutingModeDo routing_mode, FilterConditionTagDo filter_condition_tag){
        super( null, _TAG, 0, 0);
        mAidMaskDo = aid_mask;
    ...
    public void translate()
        ...
        do {
            ...
            if( temp.getTag() == AidMaskDo._TAG ) { // AID_MASK_DO
                mAidMaskDo = new AidMaskDo( data, temp.getValueIndex(), temp.getValueLength());
                mAidMaskDo.translate();
            } else if( temp.getTag() == AidRangeDo._TAG ) { // AID_RANGE_DO
                mAidRangeDo = new AidRangeDo( data, temp.getValueIndex(), temp.getValueLength());
                mAidRangeDo.translate();
            } else if( temp.getTag() == RoutingModeDo._TAG ) { // ROUTING_MODE_DO
                mRoutingModeDo = new RoutingModeDo( data, temp.getValueIndex(), temp.getValueLength());
                mRoutingModeDo.translate();
            } else if( temp.getTag() == FilterConditionTagDo._TAG ) { // FILTER_CONDITION_TAG_DO
                mFilterConditionTagDo = new FilterConditionTagDo( data, temp.getValueIndex(), temp.getValueLength());
                mFilterConditionTagDo.translate();
            } else if( temp.getTag() == VzwPermissionDo._TAG ) { // VZW_AR_DO
                mVzwArDo = new VzwPermissionDo( data, temp.getValueIndex(), temp.getValueLength());
                mVzwArDo.translate();
            } else {
        ...
        if( mAidMaskDo == null || mAidRangeDo == null ||
                mRoutingModeDo == null || mVzwArDo == null ||
