...
public final class AidFilter {
    ...
    public boolean setFilterList(byte[] filterList)
    {
        ...
        try{
            ...
            status = getServiceInterface().setVzwAidList(entries.toArray(new RouteEntry[entries.size()]));
    ...
    private void prepareRouteInfo(ClfFilterDoList all_CLF_FILTER_DO,ArrayList<RouteEntry> entries) {
        ...
        ArrayList<ClfFilterDo> clf_FILTER_DOs = all_CLF_FILTER_DO.getClfFilterDos();
        for (ClfFilterDo clf_FILTER_DO : clf_FILTER_DOs) {

            byte[] aid = getAid(clf_FILTER_DO.getFilterEntryDo().getAidRangeDo(),
                    clf_FILTER_DO.getFilterEntryDo().getAidMaskDo());

            int powerState = getPowerState(clf_FILTER_DO.getFilterEntryDo());
            Log.d("AidFilter","prepareRouteInfo powerState" + powerState);
            RouteEntry entry = new RouteEntry(aid,powerState, DEFAULT_ROUTE_LOCATION,clf_FILTER_DO.getFilterEntryDo().getVzwArDo().isVzwAllowed());
            entries.add(entry);
        }
    }
    ...
    private int getPowerState(FilterEntryDo filter_entry_do) {
        ...
        if( (conditionTagDo != null && conditionTagDo.getFilterConditionTag() ==
                FilterConditionTagDo.SCREEN_OFF_TAG)
                )
        {
            /*It is a payment AID, only enable in Screen on and FULL Power mode. */
            powerState = 0x01;//only full power mode
        } else if(routeInfo != 0x00){
            /*It is a non payment AID, can be enable in Screen off and F|L|N Power mode
              screen off enable bit on and support in low power and full power mode*/
            powerState = (0x01 << 2) | (routeInfo & 0x02)|((routeInfo & 0x04)>> 2);
        }
