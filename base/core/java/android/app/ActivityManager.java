...
//#ifndef VENDOR_EDIT
//Licx@OnLineRD.framework.key_intercept, add for key intercept.
...
public class ActivityManager {
    ...
    public static class AppTask {
	  ...
	  //#ifdef VENDOR_EDIT
    //licx@OnLineRD.framework.key_intercept, add for key interceptor
    /**
     * @hide
     */
    public final boolean fixKeyEventInterceptorInProcess(int[] keys) {
        if(keys != null){
            for (int i = 0; i < keys.length; ++i) {
                if (keys[i] < 0 && keys[i] > KeyEvent.getMaxKeyCode())
                    return false;
            }
        }
        boolean res = false;
        try {
            res = ActivityManagerNative.getDefault().fixKeyEventInterceptorInProcess(keys);
        } catch (RemoteException e) {
        }
        return res;
    }
     ...
     * @hide
     */
    public final boolean hasKeyEventInterceptor(){
        try {
            return ActivityManagerNative.getDefault().isInInterceptorMap();
        } catch (RemoteException e) {
        }
        return false;
    }
