...
public abstract class ActivityManagerNative extends Binder implements IActivityManager
{
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags)
		...
		//#ifdef VENDOR_EDIT
        //licx@OnLineRD.framework.get_topAct_comp, add
        case GET_TOP_ACTIVITY_COMPONENTNAME_TRANSACTION:{
        	data.enforceInterface(IActivityManager.descriptor);
        ...
        //for key_intercept:
        case FIX_KEYEVENT_INTERCEPTOR_IN_PROCESS_TRANSACTION: {
            data.enforceInterface(IActivityManager.descriptor);
            int N = data.readInt();
            int[] keys;
            if(N == IActivityManager.INVALIDATE_ARRAY_LENGTH)
            	keys = null;
            else{
            	keys = new int[N];
            	data.readIntArray(keys);
            }
            boolean res = fixKeyEventInterceptorInProcess(keys);
            reply.writeNoException();
            reply.writeInt(res ? 1 : 0);
            return true;
        }
        
        case CHECK_KEYEVENT_INTERCEPTOR_TRANSACTION: {
            data.enforceInterface(IActivityManager.descriptor);
            int key = data.readInt();
            boolean res = isKeyEventIntercepted(key);
            reply.writeNoException();
            reply.writeInt(res ? 1 : 0);
            return true;
        }
        
        case IS_IN_INTERCEPTOR_MAP_TRANSACTION: {
            data.enforceInterface(IActivityManager.descriptor);
            boolean res = isInInterceptorMap();
            reply.writeNoException();
            reply.writeInt(res ? 1 : 0);
            return true;
        }
...
class ActivityManagerProxy implements IActivityManager
{
    ...
    public ComponentName getTopActivityComponentName() throws RemoteException {
        ...
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        mRemote.transact(GET_TOP_ACTIVITY_COMPONENTNAME_TRANSACTION, data, reply, 0);
        reply.readException();
    ...
    //licx@OnLineRD.framework, add
    //for key_intercept:
    public boolean fixKeyEventInterceptorInProcess(int[] keys) throws RemoteException {
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        if(keys == null)
        	data.writeInt(IActivityManager.INVALIDATE_ARRAY_LENGTH);
        else{
        	data.writeInt(keys.length);
        	data.writeIntArray(keys);
        }
        mRemote.transact(FIX_KEYEVENT_INTERCEPTOR_IN_PROCESS_TRANSACTION, data, reply, 0);
        reply.readException();
        boolean res = reply.readInt() != 0;
        data.recycle();
        reply.recycle();
    ...
    public boolean isKeyEventIntercepted(int key) throws RemoteException {
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        data.writeInt(key);
        mRemote.transact(CHECK_KEYEVENT_INTERCEPTOR_TRANSACTION, data, reply, 0);
        reply.readException();
        boolean res = reply.readInt() != 0;
        data.recycle();
        reply.recycle();
    ...
    public boolean isInInterceptorMap() throws RemoteException {
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        mRemote.transact(IS_IN_INTERCEPTOR_MAP_TRANSACTION, data, reply, 0);
        reply.readException();
        boolean res = reply.readInt() != 0;
        data.recycle();
        reply.recycle();
