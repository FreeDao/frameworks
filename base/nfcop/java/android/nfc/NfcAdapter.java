...
public final class NfcAdapter {
    ...
    public static final String NONE_SE_ID = "com.nxp.none_se.ID";
    ...
    //public static final int HOST_ID = 0x400;
     ...
     * {@link android.nfc.NfcAdapter#EXTRA_AID} and {@link android.nfc.NfcAdapter#EXTRA_SOURCE}
     *
     */
    @SdkConstant(SdkConstantType.BROADCAST_INTENT_ACTION)
     ...
     * <p>
     * Always contains the extra field
     * {@link android.nfc.NfcAdapter#EXTRA_SOURCE}
     */
    @SdkConstant(SdkConstantType.BROADCAST_INTENT_ACTION)
    ...
    public void selectDefaultSecureElement(String pkg, String seId) throws IOException {
        ...
        try {
            if(sService.getSelectedSecureElement(pkg) != seID) {
                sService.deselectSecureElement(pkg);
            }
        ...
        try {
            seList = sService.getSecureElementList(pkg);
            // ADD
                ...
                if (seId.compareTo(ALL_SE_ID) != 0) {
                    for (int i = 0; i < seList.length; i++) {
                        if (seList[i] == seID) {
                            ...
                            sService.selectSecureElement(pkg, seID);
                            seSelected = true;
                        }
                    }
                } else {
            ...
            if (!seSelected) {
                if (seId.equals(NfcAdapter.UICC_ID)) {
                    sService.storeSePreference(pkg, seID);
            ...
            Log.e(TAG, "selectDefaultSecureElement: getSecureElementList failed", e);
            throw new IOException("selectDefaultSecureElement: getSecureElementList failed");
        }
    }
    /**
    ...
    */
    public INfcAccessExtras getNfcAccessExtras(String pkg) {
        try {
            return sService.getNfcAccessExtrasInterface(pkg);
        } catch (RemoteException e) {
 ...
 /**
    ...
    public String getDefaultSelectedSecureElement(String pkg) throws IOException {
        ...
        try {
            seID = sService.getSelectedSecureElement(pkg);
...
/**
     ...
     * <p>Requires {@link android.Manifest.permission#NFC} permission.
     *
     * @deprecated Use {@link NfcAdapter#deSelectedSecureElement}
    ...
    /**
     * get Available Secure Element List
     * <p>Requires {@link android.Manifest.permission#NFC} permission.
     *
    ...
    public String[] getAvailableSecureElementList(String pkg) throws IOException {
        ...
        try {
            ...
            list = sService.getSecureElementList(pkg);
            listSize = list.length;

            String[] listStrArray = new String[listSize];
            for(int i = 0; i < listSize; i ++) {
            	    if(list[i] == SMART_MX_ID_TYPE) {
            	        listStrArray[i] = SMART_MX_ID;
            	    } else if(list[i] == UICC_ID_TYPE) {
            	        listStrArray[i] = UICC_ID;
            	    } else if(list[i] == ALL_SE_ID_TYPE) {
            	        listStrArray[i] = ALL_SE_ID;
            	    } else {
            	        throw new IOException("Get Secure Element failed");
            	    }
            	}
     ...
     */
     ...
     public void MifareDesfireRouteSet(String routeLoc, boolean fullPower, boolean lowPower, boolean noPower)
         ...
         try{
             ...
             if (routeLoc.equals(UICC_ID)) {

             seID = UICC_ID_TYPE;
             } else if (routeLoc.equals(SMART_MX_ID)) {

             seID= SMART_MX_ID_TYPE;

             } else if (routeLoc.equals(HOST_ID)) {
                 seID = HOST_ID_TYPE;
             } else {
             ...
             sService.MifareDesfireRouteSet(seID, fullPower, lowPower, noPower);
             } catch (RemoteException e) {
        ...
        try {
            ...
            seID= SMART_MX_ID_TYPE;
              ...
              seID = HOST_ID_TYPE;
               ...
               sService.DefaultRouteSet(seID, fullPower, lowPower, noPower);
            } catch (RemoteException e) {
    ...
    public void MifareCLTRouteSet(String routeLoc, boolean fullPower, boolean lowPower, boolean noPower )
        ...
        try {
            ...
            sService.MifareCLTRouteSet(seID, fullPower, lowPower, noPower);
        } catch (RemoteException e) {
